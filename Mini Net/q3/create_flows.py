...
for i in range(0, pathForwardLen):
    ...
    if i == 0:
        initialFlows(switch_index=0, direction=1)
    elif i == pathForwardLen-1:
        initialFlows(switch_index=pathForwardLen - 1, direction=1)
    else:
        addRouteFlow(switch_index=i, direction=1)

for i in range(0, pathBackwardLen):
    [root, applyActions] = buildStructure()
    # direction 2 is for backward path
    if i == 0:
        initialFlows(switch_index=0, direction=2)
    elif i == pathBackwardLen - 1:
        initialFlows(switch_index=pathBackwardLen - 1, direction=2)
    else:
        addRouteFlow(switch_index=i, direction=2)
